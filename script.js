let currentOperator = '';

function setOperator(op) {
    currentOperator = op;
    const num1 = document.getElementById("num1").value;
    document.getElementById("calculation-display").innerText = `${num1} ${currentOperator}`;
}

function calculate() {
    const num1 = parseFloat(document.getElementById("num1").value);
    const num2 = parseFloat(document.getElementById("num2").value);
    const resultElement = document.getElementById("result");
    const displayElement = document.getElementById("calculation-display");
    let result = "";

    if (isNaN(num1) || isNaN(num2)) {
        result = "กรุณาใส่ตัวเลขทั้งสองช่อง";
    } else if (currentOperator === '') {
        result = "กรุณาเลือกตัวดำเนินการ (+ - × ÷)";
    } else {
        switch(currentOperator) {
            case "+": result = num1 + num2; break;
            case "-": result = num1 - num2; break;
            case "*": result = num1 * num2; break;
            case "/": result = num2 !== 0 ? num1 / num2 : "หารด้วยศูนย์ไม่ได้"; break;
        }
    }
    resultElement.innerText = result;
    displayElement.innerText += ` ${num2} =`;
}

function clearEntry() {
    document.getElementById("num1").value = '';
    document.getElementById("num2").value = '';
    document.getElementById("result").innerText = '0';
    document.getElementById("calculation-display").innerText = '';
    currentOperator = '';
}